function [lh_labels, rh_labels] = CBIG_ArealMSHBM_gMSHBM_generate_individual_parcellation(project_dir, ...
...
%
% This script will estimate the gradient-based areal-level individual parcellation 
...
% smoothness prior should be specified by c, the weight of gradient-based spatial
...
%        each test subject.
%
%     3) project_dir/gradient_list/<subject_set>/gradient_list_lh.txt
%        project_dir/gradient_list/<subject_set>/gradient_list_rh.txt
%        contain the diffusion embedding matrices of RSFC gradient maps of
%        the left and right hemisphere for subjects generated by all
%        sessions. Each list should contain S rows, where each row 
%        is the full file path of the gradient file for each test subject.
%
...
%   - beta: (string)
%
%     The weight of gradient-based spatial localization prior. For example, 
%     '100'. A large beta indicates stronger weight of the gradient map. The
%     estimated individual areal-level parcellation will be very similar to 
%     the gradient-based edge-detection parcellation (Gordon et al. 2016).
%
%   - subject_set: (string)
%
...
%
% Example:
% [lh_labels, rh_labels] = CBIG_ArealMSHBM_gMSHBM_generate_individual_parcellation(project_dir,'fs_LR_32k','2', ...
% '400','1','30','30','100','test_set')
%
...
% weight for gradient-based connectedness prior
setting_params.beta = repmat(str2double(beta)*1000,1,setting_params.num_clusters); 
setting_params.epsilon = 1e-4;

setting_params.connect_th = 15;
%% read group priors
group_prior_file = fullfile(project_dir, 'priors', 'gMSHBM', ['beta' beta], 'Params_Final.mat');   
...
boundary_mask = [lh_boundary zeros(size(rh_boundary)); zeros(size(lh_boundary)) rh_boundary];
...
out_dir = fullfile(project_dir,'ind_parcellation_gMSHBM', subject_set, ...
...
function Params = check_connectedness(Params, setting_params)
    ...
    % get current labels
    labels = zeros(1,size(Params.s_lambda,1));
    ...
    parcel_components = CBIG_ArealMSHBM_compute_components_general(lh_labels, rh_labels,setting_params.mesh,...
        setting_params.num_clusters);
    eucli_dist = CBIG_ArealMSHBM_component_distance(lh_labels, rh_labels,setting_params.mesh,...
        setting_params.num_clusters);
    distribute_parcels = find((eucli_dist > connect_th) | (parcel_components > 3));
    distribute_connectedness = eucli_dist((eucli_dist > connect_th) | (parcel_components > 3));
...
while(stop_em == 0)
    ...
    Params = spatial_connect_prior(data, Params, setting_params);
    %Params = spatial_xyz_prior(Params);
    %% Estep
    fprintf('Estep..\n');
    ...
    while stop_comp2 == 0
        ...
        while stop_lambda == 0
            ...
            log_vmf = log_vmf + setting_params.beta.*Params.spatial_connect_vmf + Params.spatial_xyz_vmf; 
        ...
        if((Params.max_connectedness <= setting_params.connect_th) && (Params.max_components <= 3)) 
            stop_comp2 = 1;
        end
        if(comp_iter2 >= 15) 
            stop_comp2 = 1;
    ...
    for s = 1:setting_params.num_sub    
        ...
        Params.spatial_connect_vmf(isnan(Params.spatial_connect_vmf)) = log(eps.^20);
        Params.spatial_connect_vmf(isinf(Params.spatial_connect_vmf)) = log(eps.^20);
        ...
        update_cost(:,s) = sum(sum(s_lambda_cost.*log_lambda_prop)) + ...
                           ...
                           sum(sum(setting_params.beta.*s_lambda_cost.*Params.spatial_connect_vmf(:,:,s)));    
    end
...
function Params = spatial_connect_prior(data, Params, setting_params)
% compute xyz prior
grad_data = data.gradient_mat;
u_update = transpose(grad_data)*Params.s_lambda;
Params.u = transpose(bsxfun(@rdivide, u_update,sum(Params.s_lambda,1)));

for p = 1:setting_params.num_clusters
    curr_parcel_diff = bsxfun(@minus,grad_data,Params.u(p,:));
    
    curr_spatial_vmf(:,p) = -1*sum(curr_parcel_diff.^2,2);
end
curr_spatial_vmf(isnan(curr_spatial_vmf)) = -Inf;
Params.spatial_connect_vmf = curr_spatial_vmf;
Params.spatial_connect_vmf(1:setting_params.num_verts/2, ...
                            setting_params.num_clusters/2+1:setting_params.num_clusters) = -Inf;
Params.spatial_connect_vmf(setting_params.num_verts/2 + 1:end,1:setting_params.num_clusters/2) = -Inf;

end

function data = fetch_data(project_dir,num_session,subid,mesh,subject_set)
% read in gradient
lh_data_gradient = fullfile(project_dir, 'gradient_list', subject_set, ['gradient_list_lh.txt']);
rh_data_gradient = fullfile(project_dir, 'gradient_list', subject_set, ['gradient_list_rh.txt']);
...
lh_gradient_file = matfile(lh_gradient_name{subid});
rh_gradient_file = matfile(rh_gradient_name{subid});

lh_gradient_mat = lh_gradient_file.emb(:,1:100);
rh_gradient_mat = rh_gradient_file.emb(:,1:100);
data.gradient_mat = [lh_gradient_mat; rh_gradient_mat];
