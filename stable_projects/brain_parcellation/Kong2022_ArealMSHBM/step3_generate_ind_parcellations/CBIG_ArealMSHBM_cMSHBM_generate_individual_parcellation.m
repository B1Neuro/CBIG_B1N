function [lh_labels, rh_labels] = CBIG_ArealMSHBM_cMSHBM_generate_individual_parcellation(project_dir,...
     mesh, num_sess, num_clusters, subid, w, c, beta, subject_set)

% [lh_labels, rh_labels] = CBIG_ArealMSHBM_cMSHBM_generate_individual_parcellation ...
% (project_dir, mesh, num_sess, num_clusters, subid, w, c, beta, subject_set)
%
% This script will estimate the strictly contiguous areal-level individual parcellation 
% with num_clusters parcels for a single subject using num_sess sessions. The
% weight of group spatial prior should be specified by w, the weight of MRF
% smoothness prior should be specified by c, the weight of xyz spatial
% localization prior should be specified by beta.
%
% To generate the individual areal-level parcellation, we assume the group priors
% are already estimated by running CBIG_ArealMSHBM_estimate_group_priors.m, or 
% the pre-computed group priors stored in lib/group_priors. We currently provide
% pre-computed group priors in fsaverage6 and fs_LR_32k space. The estimated group 
% priors should be saved in project_dir/priors/Params_Final.mat.
%
...
%   - beta: (string)
%
%     The weight of xyz spatial localization prior. For example, 
%     '100'. A large beta indicates stronger weight of the xyz prior. The
%     estimated individual areal-level parcellation will be very round with
%     very large beta.
%
%   - subject_set: (string)
%
...
%
% Example:
% [lh_labels, rh_labels] = CBIG_ArealMSHBM_cMSHBM_generate_individual_parcellation(project_dir,'fs_LR_32k',...
%   '2','400','1','30','30','100','test_set')
%
...
setting_params.beta = repmat(str2double(beta),1,setting_params.num_clusters); % weight for xyz connectedness prior
setting_params.epsilon = 1e-4;

setting_params.connect_th = 0;
%% read group priors
group_prior_file = fullfile(project_dir, 'priors', 'cMSHBM', ['beta' beta], 'Params_Final.mat');   
...
% subjects.
Params.s_psi = repmat(Params.mu,1,1,setting_params.num_sub);
...
Params.xyz_gamma = zeros(1,setting_params.num_clusters);
Params.max_connectedness = 0;
Params.max_components = 0;
...
Params.s_t_nu = repmat(Params.mu,1,1,setting_params.num_session,setting_params.num_sub);

% s_lambda: NxLxS. The posterior probability of the individual-specific
% parcellation of each subject. 
Params.s_lambda = Ini.Params.theta;
Params.spatial_xyz_vmf= zeros(size(Params.s_lambda,1),setting_params.num_clusters);
...
%obtain non-zero elements in theta
tmp_theta = Params.theta(sum(Params.s_lambda,2)~=0,:);
[setting_params.row_idx,setting_params.col_idx] = ind2sub(size(tmp_theta),find(tmp_theta ~= 0));
setting_params.row_idx = setting_params.row_idx -1;
setting_params.col_idx = setting_params.col_idx -1;
...
while(stop_intra_em == 0)
    ...
    fprintf('It is intra em iteration %d..\n',iter_intra_em);
    Params.kappa = ini_val*ones(1,setting_params.num_clusters);
    Params.s_t_nu = repmat(Params.mu,1,1,setting_params.num_session,setting_params.num_sub);
    Params = vmf_clustering_subject_session(Params,setting_params,data,boundary_mask);
...
lh_labels = CBIG_RemoveIsolatedSurfaceComponents(lh_avg_mesh, lh_labels, 5);
rh_labels = CBIG_RemoveIsolatedSurfaceComponents(rh_avg_mesh, rh_labels, 5);

out_dir = fullfile(project_dir,'ind_parcellation_cMSHBM', subject_set, ...
          [num2str(setting_params.num_session) '_sess'], ['beta' beta]);
...
save(fullfile(out_dir, ...
    ...
    c,'_beta',beta,'.mat']), 'lh_labels','rh_labels');
...
%%%%% sub-functions
%%%%%%%%%%%%%%%%%%%%%%%%
function Params = check_connectedness(Params, setting_params)

% set connectedness threshold
connect_th = setting_params.connect_th;
% get current labels
[~,labels(sum(Params.s_lambda,2)~=0)]=max(Params.s_lambda(sum(Params.s_lambda,2)~=0,:,1),[],2);
lh_labels = labels(1:length(labels)/2)';
rh_labels = labels((length(labels)/2 + 1):end)';

mesh = setting_params.mesh;
...
parcel_components = CBIG_ArealMSHBM_compute_components_general(lh_labels, rh_labels,...
    setting_params.mesh, setting_params.num_clusters);
eucli_dist = CBIG_ArealMSHBM_component_distance(lh_labels, rh_labels,setting_params.mesh, setting_params.num_clusters);
distribute_parcels = find((eucli_dist > connect_th) | (parcel_components > 1));
distribute_connectedness = eucli_dist((eucli_dist > connect_th) | (parcel_components > 1));
if(isempty(distribute_parcels))
    Params.max_connectedness = 0;
    Params.max_components = 1;
else    
    Params.max_connectedness = max(distribute_connectedness);
    Params.max_components = max(parcel_components);
end

%XYZ prior
Params.xyz_gamma(distribute_parcels) = Params.xyz_gamma(distribute_parcels) + 1000;

end


function Params = spatial_xyz_prior(Params,setting_params)
...
lambda_X = spatial_data'*s_lambda;
Params.s_muc = bsxfun(@rdivide,lambda_X,sqrt(sum((lambda_X).^2)));

gamma_tmp=spatial_data*Params.s_muc; % 64984x3 * 3x400
spatial_xyz_vmf_tmp = gamma_tmp;
Params.spatial_xyz_vmf = bsxfun(@plus,Cdln(Params.xyz_gamma,3),bsxfun(@times,Params.xyz_gamma,spatial_xyz_vmf_tmp));
Params.spatial_xyz_vmf(isnan(Params.spatial_xyz_vmf)) = 0;
...
s_psi_update = sum(bsxfun(@times,Params.s_t_nu, ...
               repmat(Params.sigma,size(Params.s_t_nu,1),1,size(Params.s_t_nu,3),size(Params.s_t_nu,4))),3);
s_psi_update = reshape(s_psi_update,size(s_psi_update,1), ...
               size(s_psi_update,2),size(s_psi_update,3)*size(s_psi_update,4));
...
function Params = vmf_clustering_subject_session(Params,setting_params,data,boundary_mask)
...
while(stop_em == 0)
    ...
    while(stop_m == 0)
        ...
        if(iter_m > 50)
            stop_m = 1;
            warning('M step can not converge');
        end
        kappa_update(1)
        Params.kappa = kappa_update;
    end
    
    Params = spatial_xyz_prior(Params, setting_params);
    ...
    while stop_comp2 == 0
        ...
        while stop_lambda == 0
            ...
            % add spatial connectness cost
            log_vmf = log_vmf + setting_params.beta.*Params.spatial_xyz_vmf; 
        ...
        if(iter_em > 0)
            Params = check_connectedness(Params, setting_params);
            Params = spatial_xyz_prior(Params,setting_params);
            disp(['Connectedness ' num2str(Params.max_connectedness) ' components ' num2str(Params.max_components)]);
        else
            stop_comp2 = 1;
        end
        if((Params.max_connectedness <= setting_params.connect_th) && (Params.max_components <= 1)) 
            stop_comp2 = 1;
        end
        if(comp_iter2 >= 15) 
            stop_comp2 = 1;
            disp('conenctedness cannot converge \n')
        end
    end
    ...
    for s = 1:setting_params.num_sub    
        ...
        Params.spatial_xyz_vmf(isnan(Params.spatial_xyz_vmf)) = log(eps.^20);
        Params.spatial_xyz_vmf(isinf(Params.spatial_xyz_vmf)) = log(eps.^20);

        update_cost(:,s) = sum(sum(s_lambda_cost.*log_lambda_prop)) + ...
                           sum(sum(s_lambda_cost.*log_theta_cost)) - ...
                           sum(sum(s_lambda_cost.*setting_params.w.*log_s_lambda_cost)) - ...
