function Params = CBIG_ArealMSHBM_dMSHBM_estimate_group_priors_parent(project_dir, mesh, ...
     num_sub, num_sess, num_clusters, tmp_dir, max_iter)

% Params = CBIG_ArealMSHBM_dMSHBM_stimate_group_priors_parent(project_dir,mesh,num_sub, ...
% num_sess,num_clusters,tmp_dir,max_iter)
%
% This is the parent script to estimate group priors for distributed individual
...
%
% Example:
%   Params = CBIG_ArealMSHBM_dMSHBM_estimate_group_priors_parent(project_dir,...
%   'fsaverage6','3','4','400','/project_dir/tmp_results');
%
...
while(stop_inter == 0)
    ...
    if(abs(abs(update_cost_inter-cost_inter)./cost_inter) <= 1e-5 || Params.iter_inter >= max_iter)
        ...
        Params.s_psi = [];
        Params.s_t_nu = [];
        save(fullfile(project_dir, 'priors', 'dMSHBM', 'Params_Final.mat'),'Params','-v7.3');
        fprintf('Finished!\n');
    end
    ...
    mkdir(fullfile(project_dir, 'priors', 'dMSHBM'));
    save(fullfile(project_dir, 'priors', 'dMSHBM',['Params_iteration' num2str(Params.iter_inter)...
        '.mat']),'Params','-v7.3');
end
...
while(stop_em == 0)
    ...
    save([setting_params.tmp_dir '/inter_' num2str(Params.iter_inter) '/intra_'...
        num2str(Params.iter_intra) '/EMstep_iter' num2str(iter_em) '/cost/EMiter'...
        num2str(iter_em) '_cost.mat'],'update_cost');
    save_dump([setting_params.tmp_dir '/inter_' num2str(Params.iter_inter) '/intra_'...
