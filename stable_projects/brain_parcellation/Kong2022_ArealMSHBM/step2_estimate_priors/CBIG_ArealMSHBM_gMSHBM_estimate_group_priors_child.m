function CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_child(s, mesh, tmp_dir)

% CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_child(s, mesh, tmp_dir)
%
% This is the child script to estimate group priors for gradient-based individual
...
% num_sub child jobs.
% Check CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_parent.m for more details.
%
% Input:
%
%   - s: (string)
...
%
% Example:
%   CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_child('1', 'fs_LR_32k', tmp_dir)
%
...
if(~isempty(strfind(mesh,'fs_LR_32k')))
    ...
    for t = 1:setting_params.num_session
        ...
        lh_gradient_file = matfile(lh_gradient_list{s,t});
        rh_gradient_file = matfile(rh_gradient_list{s,t});
        data_gradient_mat(:,:,t) = [lh_gradient_file.emb(:,1:100); rh_gradient_file.emb(:,1:100)];
        clear lh_gradient_mat rh_gradient_mat;
    ...
    while(1)
        ...
        while(1)
            ...
            while(1)
                ...
                iter_m = iter_m + 1;
                Mstep_params_tmp = load_until_exist([tmp_dir '/Mstep/setting_params/sub' num2str(s) '_'...
                     num2str(iter_m) '.mat']);
            ...
            for t=1:setting_params.num_session
                grad_data = data_gradient_mat(:,:,t);
                s_u_update = transpose(grad_data)*Mstep_params_tmp.tmp_s_lambda;
                s_u = transpose(bsxfun(@rdivide, s_u_update,sum(Mstep_params_tmp.tmp_s_lambda,1)));

                for p = 1:setting_params.num_clusters
                    curr_parcel_diff = bsxfun(@minus,grad_data,s_u(p,:));
                    curr_spatial_vmf(:,p,t) = -1*sum(curr_parcel_diff.^2,2);
                end
            end
            log_connect_vmf = sum(curr_spatial_vmf,3);
            log_connect_vmf(1:size(lh_avg_mesh.vertices,2),...
                setting_params.num_clusters/2+1:setting_params.num_clusters,:) = -Inf;
            log_connect_vmf(size(lh_avg_mesh.vertices,2) + 1:end,1:setting_params.num_clusters/2,:) = -Inf;
            ...
            if(~exist([tmp_dir '/Estep/converge_flag/Estep_cost.mat.dump']))
                ...
                %% add gradient-based connectness prior
            ...
            EMstep_iter = load_until_exist([tmp_dir '/inter_' num2str(iter_inter_em) '/intra_'...
                 num2str(iter_intra_em) '/EMstep_iter' num2str(iter_em) '/converge_flag/iter_'...
                 num2str(iter_em) '.mat']);
