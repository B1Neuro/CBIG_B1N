function Params = CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_parent(project_dir, mesh, ...
     num_sub, num_sess, num_clusters, beta, tmp_dir, max_iter)

% Params = CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_parent(project_dir,mesh,num_sub, ...
% num_sess,num_clusters,beta,tmp_dir, max_iter)
%
% This is the parent script to estimate group priors for gradient-based individual
...
%        each test subject.
%
%     3) project_dir/gradient_list/training_set/gradient_list_lh.txt
%        project_dir/gradient_list/training_set/gradient_list_rh.txt
...
%
% Example:
%   Params = CBIG_ArealMSHBM_gMSHBM_estimate_group_priors_parent(project_dir,...
...
setting_params.beta = str2double(beta)*1000;
...
while(stop_inter == 0)
    ...
    while(stop_intra_em == 0)
        ...
        update_cost = sum(sum(sum(update_cost,2),3),4)+sum(sum(bsxfun(@plus,sum(bsxfun(@times,...
            bsxfun(@times,Params.mu,Params.s_psi),Params.epsil),1),Cdln(Params.epsil,setting_params.dim)),2),3);
        ...
        if(~exist([setting_params.tmp_dir '/inter_' num2str(Params.iter_inter) '/intra_'...
            num2str(iter_intra_em) '/converge_flag']))
    ...
    if(abs(abs(update_cost_inter-cost_inter)./cost_inter) <= 1e-5 || Params.iter_inter >= max_iter)
        ...
        Params.s_psi = [];
        Params.s_t_nu = [];
        save(fullfile(project_dir, 'priors', 'gMSHBM',['beta' beta], 'Params_Final.mat'),'Params','-v7.3');
        fprintf('Finished!\n');
    end
    ...
    mkdir(fullfile(project_dir, 'priors', 'gMSHBM',['beta' beta]));
    save(fullfile(project_dir, 'priors', 'gMSHBM',['beta' beta], ['Params_iteration'...
...
while(stop_intra == 0)
    ...
    s_psi_update = sum(bsxfun(@times,Params.s_t_nu,repmat(Params.sigma,...
        size(Params.s_t_nu,1),1,size(Params.s_t_nu,3),size(Params.s_t_nu,4))),3);
...
while(stop_em == 0)
    ...
    while(stop_m == 0)
        ...
        for s = 1:setting_params.num_sub
            ...
            save([setting_params.tmp_dir '/Mstep/setting_params/sub' num2str(s) '_'...
                num2str(iter_m) '.mat'], 'tmp_s_lambda','tmp_psi','tmp_sigma','tmp_s_t_nu');
            save_dump([setting_params.tmp_dir '/Mstep/setting_params/sub' num2str(s) '_'...
                num2str(iter_m) '.mat']);
        end
        
        for s = 1:setting_params.num_sub
            kappa_tmp = load_until_exist([setting_params.tmp_dir '/Mstep/kappa_step1/sub' num2str(s) '_'...
                num2str(iter_m) '.mat']);
        ...
        for s = 1:setting_params.num_sub
            tmp_s_t_nu = load_until_exist([setting_params.tmp_dir '/Mstep/s_t_nu/sub' num2str(s) '_'...
                num2str(iter_m) '.mat']);
